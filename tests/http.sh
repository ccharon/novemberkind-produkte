...
check 'Auswahl der Produktart zeigt 5 Arten' "$(curl -s -b "$JAR" "$APP/neu/" | grep -c 'class="ep-type"')" 5
page=$(curl -s -b "$JAR" "$APP/neu/button/")
nonce=$(grep -oP 'var easyProduct = .*?"nonce":"\K[a-f0-9]+' <<<"$page")
...
check 'Übersicht nach Kategorien' "$(grep -oP 'class="ep-group__title">\K[^<]+' <<<"$overview" | paste -sd,)" 'Buttons,Karten,magnetische Lesezeichen,Originalzeichnungen,Sticker,Tassen'
...
check 'Menüeintrag im Backend leitet weiter' "$(location -b "$JAR" "$BASE/wp-admin/admin.php?page=easy-product")" "$APP/"
...
check 'Vorschau der Beschreibung' "$(ajax -d action=easy_product_preview -d "nonce=$nonce" -d type=sticker --data-urlencode 'motif=Eule' -d finish=glaenzend -d width=7,5 -d height=6)" 200
check 'Vorschau enthält Motiv und Maße' "$(json "'Sticker Eule' in d['data']['html'] and 'Breite: 7,5 cm' in d['data']['html']")" True
check 'Vorschlag ohne Nonce abgelehnt' "$(ajax -d action=easy_product_suggest -d type=card -d motif=x)" 403
check 'Speichern ohne Nonce abgelehnt' "$(ajax -d action=easy_product_save -d type=card -d motif=x -d price=1)" 403
check 'unbekannte Produktart abgelehnt' "$(ajax -d action=easy_product_save -d "nonce=$nonce" -d type=tasse -d motif=x)" 400
check 'Pflichtfehler liefert 422' "$(ajax -d action=easy_product_save -d "nonce=$nonce" -d type=card -d motif= -d price=)" 422
...
check 'Upload erfolgreich' "$(ajax -F action=easy_product_upload -F "nonce=$nonce" -F "file=@$TMP/foto.png;type=image/png")" 200
image_id=$(json "d['data']['id']")
check 'Upload liefert WebP-Adresse' "$(json "d['data']['url'].endswith('.webp')")" True
check 'Upload ohne Datei abgelehnt' "$(ajax -F action=easy_product_upload -F "nonce=$nonce")" 400

check 'Speichern erfolgreich' "$(ajax -d action=easy_product_save -d "nonce=$nonce" -d type=card --data-urlencode 'motif=HTTP-Test' -d "sku=$(bin/wp eval 'echo EasyProduct\ShopData::next_sku();')" \
...
check 'Ändern erfolgreich' "$(ajax -d action=easy_product_save -d "nonce=$nonce" -d type=card -d "product_id=$product_id" -d "sku=$sku" \
...
check 'Ändern mit falscher Produktart abgelehnt' "$(ajax -d action=easy_product_save -d "nonce=$nonce" -d type=sticker -d "product_id=$product_id" \
